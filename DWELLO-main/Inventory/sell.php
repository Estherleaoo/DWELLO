<?php
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        $qnt_venda = $_POST['qnt_venda'];

        $sql = "UPDATE produtos SET nome = '$nome', valor = '$valor', cor='$cor', material = '$material', tamanho = '$tamanho', formato = '$formato', peso = '$peso', qnt_estoque = '$qnt_estoque', qnt_venda = '$qnt_venda' WHERE codigo = '$codigo'";
        echo $sql; 
    ...
    <div class="container p-3 mb-2 bg-light text-dark" style="border-radius: 10px;">
        <?php if($row): ?>
            ...
            <div>
                ...
                <input type="number" maxlength="30" value="<?php echo $row['qnt_estoque']?>" name="qnt_estoque" disabled>
            </div>
            <div>
                <label for="qnt_venda">Quantidade a ser vendida</label>
                <input type="number" maxlength="30" value="<?php echo $row['qnt_venda']?>" name="qnt_venda"> <!-- CORRIGIR AQUI -->
            </div>
